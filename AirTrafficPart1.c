...
void *air_traffic_control(void* arg){				// Tower thread
	...
	while(time(NULL)<= begtime+simtime){										// Tower checks if there are any planes in the landing queue and then sends signal
		pthread_mutex_lock(atc_main_lock);										// to the first plane in the queue to get ready
		pthread_cond_wait(atc_main_cond, atc_main_lock); 						// It favors the landing planes for Part1
		...
		}else if(Landing_Queue->size == 0 && TakeOff_Queue->size>0){ 			// If there is no landing queue and there are planes
			plane* ready = get(TakeOff_Queue);						 			// waiting to depart, then tower sends signal
			printf("Takeoff plane with ID: %d can depart now\n", ready->ID);
			pthread_mutex_lock(ready->p_lock);
			pthread_cond_signal(ready->cond);						 			// to the first plane in the queue to get ready
			pthread_cond_wait(ready->cond, ready->p_lock);	
			pthread_mutex_unlock(ready->p_lock);
		}
...
int main(int argc, char *argv[]){			// Initially we created two planes one for landing (thy) and one for departing (pegasus)
	...
	int i;									// Code for taking command arguments
	for(i=0; i<argc; i++){
