...
void *air_traffic_control(void* arg){		// Tower thread
	...
	while(time(NULL)<= begtime+simtime){		
		...
		if(Emergency_Queue->size>0){					// Tower checks if there are any planes in the emergency queue and then sends signal
			plane* ready = get(Emergency_Queue); 		
			printf("EMERGENCY Landing plane with ID: %d must land immediately!!!\n", ready->ID);
		...
		}else if(TakeOff_Queue->size>0 && (Landing_Queue->size==0 || TakeOff_Queue->size>=3 || (time(NULL)-begtime-(TakeOff_Queue->first->now->arrival_time)) >= 10*t)){	
			plane* ready = get(TakeOff_Queue);						 
			printf("Takeoff plane with ID: %d can depart now\n", ready->ID);	// To protect our simulation from starvation
			pthread_mutex_lock(ready->p_lock);									// we added new checks according to the requirements
			pthread_cond_signal(ready->cond);						
			pthread_cond_wait(ready->cond, ready->p_lock);	
			pthread_mutex_unlock(ready->p_lock);
